window.SUI_EMPERORS = [
  {
    "dynastyId": "sui",
    "phase": "隋初建国",
    "title": "文帝",
    "name": "杨坚",
    "names": [
      "杨坚",
      "隋文帝"
    ],
    "reign": "581-604",
    "position": "隋朝建立者和制度重建核心人物，完成北方政权重组并灭陈统一。",
    "keyEvents": [
      "杨坚代周建隋",
      "开皇之治与制度重建",
      "隋灭陈与南北统一",
      "隋代科举开端"
    ],
    "relatedEventIds": [
      "sui-founds-dynasty",
      "sui-kaihuang-governance",
      "sui-unifies-china",
      "sui-imperial-exam-beginnings"
    ],
    "plainText": "读文帝，重点看他如何把北周基础改造成隋朝，并通过制度和灭陈完成南北统一。",
    "source": "《资治通鉴》卷175-177，待逐条核原文",
    "sourceUrl": "https://zh.wikisource.org/wiki/資治通鑑/卷175"
  },
  {
    "dynastyId": "sui",
    "phase": "大业扩张",
    "title": "炀帝",
    "name": "杨广",
    "names": [
      "杨广",
      "隋炀帝",
      "晋王"
    ],
    "reign": "604-618",
    "position": "大业扩张核心人物，推动东都、运河和高句丽战争，也加速隋末危机。",
    "keyEvents": [
      "隋代科举开端",
      "大运河与东都洛阳",
      "三征高句丽",
      "杨玄感起兵",
      "隋末大起义与群雄割据",
      "江都兵变与隋亡"
    ],
    "relatedEventIds": [
      "sui-imperial-exam-beginnings",
      "sui-grand-canal-luoyang",
      "sui-goguryeo-campaigns",
      "sui-yang-xuangan-rebellion",
      "sui-late-rebellions",
      "sui-jiangdu-mutiny-fall"
    ],
    "plainText": "读炀帝，不能只看暴君标签，要看国家动员能力、工程、远征和社会承受力如何一起失衡。",
    "source": "《资治通鉴》卷180-185，待逐条核原文",
    "sourceUrl": "https://zh.wikisource.org/wiki/資治通鑑/卷180"
  },
  {
    "dynastyId": "sui",
    "phase": "隋亡唐兴",
    "title": "恭帝",
    "name": "杨侑",
    "names": [
      "杨侑",
      "隋恭帝"
    ],
    "reign": "617-618",
    "position": "李渊入关后拥立的过渡皇帝，是关中隋唐禅代的合法性节点。",
    "keyEvents": [
      "隋末大起义与群雄割据",
      "江都兵变与隋亡"
    ],
    "relatedEventIds": [
      "sui-late-rebellions",
      "sui-jiangdu-mutiny-fall"
    ],
    "plainText": "读恭帝，重点看他不是实际强势皇帝，而是隋唐转换中被用来完成政权过渡的名义中心。",
    "source": "《资治通鉴》卷185，待逐条核原文",
    "sourceUrl": "https://zh.wikisource.org/wiki/資治通鑑/卷185"
  },
  {
    "dynastyId": "sui",
    "phase": "隋亡唐兴",
    "title": "皇泰主",
    "name": "杨侗",
    "names": [
      "杨侗",
      "皇泰主"
    ],
    "reign": "618-619",
    "position": "洛阳方面拥立的隋宗室，代表隋朝中央崩溃后的东都余波。",
    "keyEvents": [
      "江都兵变与隋亡"
    ],
    "relatedEventIds": [
      "sui-jiangdu-mutiny-fall"
    ],
    "plainText": "读杨侗，重点看隋亡不是全国同一时刻完成，洛阳仍有残余政权和名义竞争。",
    "source": "《资治通鉴》卷185-186，待逐条核原文",
    "sourceUrl": "https://zh.wikisource.org/wiki/資治通鑑/卷185"
  },
  {
    "dynastyId": "sui",
    "phase": "开皇政治",
    "title": "太子",
    "name": "杨勇",
    "names": [
      "杨勇",
      "太子杨勇"
    ],
    "reign": "太子时期",
    "position": "隋文帝长子，废太子事件核心人物。",
    "keyEvents": [
      "废太子杨勇与杨广上位"
    ],
    "relatedEventIds": [
      "sui-crown-prince-change"
    ],
    "plainText": "按主线关键人物阅读，不做无关百科扩写。",
    "source": "《资治通鉴》相关卷，待逐条核原文",
    "sourceUrl": "https://zh.wikisource.org/wiki/資治通鑑"
  }
];

(() => {
  const parentByChild = { "sui-founds-dynasty": "sui-kaihuang-governance", "sui-kaihuang-code-and-local-rule": "sui-kaihuang-governance", "sui-imperial-exam-beginnings": "sui-grand-canal-luoyang", "sui-daye-tours-and-frontier-display": "sui-grand-canal-luoyang", "sui-crown-prince-change": "sui-jiangdu-mutiny-fall", "sui-goguryeo-campaigns": "sui-jiangdu-mutiny-fall", "sui-yang-xuangan-rebellion": "sui-jiangdu-mutiny-fall", "sui-late-rebellions": "sui-jiangdu-mutiny-fall", "sui-wagang-luoyang-crisis": "sui-jiangdu-mutiny-fall" };
  window.SUI_EMPERORS = window.SUI_EMPERORS.map((item) => ({ ...item, relatedEventIds: [...new Set(item.relatedEventIds.map((id) => parentByChild[id] || id))], keyEvents: [...new Set(item.keyEvents.map((id) => parentByChild[id] || id))] }));
})();
